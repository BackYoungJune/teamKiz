...
public class yPlayerMovement : MonoBehaviour
{
    ...
    {
        // 점프시 로직
        if (playerInput.jump && jumpCount < 1 && dodgeCount < 1)
        {
    ...
    void Dodge()
    {
        ...
        if (playerInput.dodge && dodgeCount < 1 && jumpCount < 1)
        {
            // 닷지시 로직
            dodgeVec = MoveVec;
            moveSpeed *= 4;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if ((collision.gameObject.tag == "Floor" || collision.gameObject.tag == "Ground"|| collision.gameObject.tag == "Wall") && collision.contacts[0].normal.y > 0.7f)
        {
            // 점프 false, 점프 카운트 초기화
            isAir = false;
            jumpCount = 0;
